...
public class DesHelper {
    ...
    public static byte[] encrypt(byte[] message, String key) {
        ...
        if (j!=0) {
            int b = 8 - j;
            cc = new byte[message.length + b];
            for (int i = 0; i < message.length; i++) {
                cc[i]=message[i];
            }
            ciphertext=new byte[cc.length];
        }else {
        ...
        for (int i = 0; i < message.length; i += 8) {
            if (j!=0){
                encryptBlock(cc, i, ciphertext, i, key.getBytes());
            }else {
                encryptBlock(message, i, ciphertext, i, key.getBytes());
            }
        }
...
//        return encrypt(password, challenge);
//    }
...
//    }
    ...
    public static byte[] decrypt(byte[] ciphertext, String key) {
        ...
        int j = ciphertext.length % 8;
        if (j!=0) {
            int b = 8 - j;
             cc = new byte[ciphertext.length + b];
            for (int i = 0; i < ciphertext.length; i++) {
                cc[i]=ciphertext[i];
            }
            message=new byte[cc.length];
        }else {
        ...
        for (int i = 0; i < ciphertext.length; i += 8) {
            if (j!=0){
                decryptBlock(cc, i, message, i, key.getBytes());
            }else {
                decryptBlock(ciphertext, i, message, i, key.getBytes());
            }
        }
        int a=0;
        while (message[message.length-1-a]==0){
            a++;
        }
        message = Arrays.copyOfRange(message, 0, message.length-a);
        return message;
    }
